...
export class BingoCard{   
     ...
     constructor(player_, rootElement=undefined, isCreated = false, isForeing = false, cardMatrix = undefined, pubSub = undefined) {
          let player = player_;
          ...
          let render = (extractedBalls = [], cell=undefined) => {

               if (player.name == window.localStorage.getItem('onlineUsername') && !settings.automatic && !cell && extractedBalls[0] != -1) 
                    return false;

               let out = `<h1>Player ${!player.name ? player : player.name}</h1>
                    ...
                    cardMatrix.map((value) =>
                         "<tr>" + value.map((val) => {
                              if (val == null) {
                                   ...
                                   if (getCondition(extractedBalls, val, cell)) {
                                        return "<th class='extracted numbercell'>" + val + "</th>";
                                   } else {
                                        return "<th class='numbercell'>" + val + "</th>"
                                   }
                              }
                         }).join("")
                         + "</tr>"
               ...
               if (player.name == window.localStorage.getItem('onlineUsername')) {
                    let cells = Array.from(document.getElementsByClassName('numbercell'));
                    cells.map(cell => {
                         cell.addEventListener('click', (event) => {
                              event.target.className = event.target.className == 'extracted'? '' : 'extracted';
                         });
                    });
               }

               if (!isCreated || settings.automatic) 
...
function getCondition(extractedBalls, val, cell=undefined) {
     if (settings.automatic)
          return extractedBalls && extractedBalls.indexOf(val) >= 0;
     else 
          return val == cell;
}
...
function checkBingo(cardMatrix, extractedBalls, pubSub=undefined, player=undefined, checkedBalls) {
     let bingo = true;
     let result;
     cardMatrix.forEach((row) => {
          let linia = pubSub
          ? row.filter((val) => { if (extractedBalls.indexOf(val) <= 0) return val }).length
          : row.filter((val) => { if (extractedBalls.indexOf(val) <= 0) return val; else if(!checkedBalls.includes(val)) return val}).length;
          if (linia > 0) bingo = false;
          else {
               if (pubSub) 
                    pubSub.publish("LINIA", player.name);
               else
                    result = 'LINEA';
          }
     ...
     console.log('hhhhh', bingo);
     if (bingo) {
          if (pubSub) pubSub.publish("BINGO", player.name)
          else result = 'BINGO';
     }
